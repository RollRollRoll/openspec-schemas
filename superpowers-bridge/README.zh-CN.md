...
# cat /tmp/oss/superpowers-bridge/templates/adopters/CLAUDE.md.fragment.zh-CN.md  # 简中
...
如果你的项目早已在 `openspec/schemas/superpowers-bridge/` 安装过本 schema,想拿到最新版本,执行下列其中一种升级方式。升级会整个覆盖 `superpowers-bridge/` 目录,并提供 CLAUDE.md 片段更新 —— 详情见下方「升级会【覆盖】什么?」。
...
> `<locale>` 默认 `zh-CN`(若你 CLAUDE.md 是简中)或省略(英文)。Claude 会按你 CLAUDE.md 的现有语言判断。
...
> bridge 目录是 monolithic —— 要么整包换新版,要么留旧版,**没有逐文件 opt-in**。CLAUDE.md 是升级流程唯一会碰项目根目录的文件,且永远等你 ack。
...
- **在 `config.yaml` 加自定义字段**(例如 `skill_bindings`):OpenSpec CLI 不识别这些字段,没有校验、没有发现性,且需要修改多个 SKILL.md 才能读取。
- **直接修改 opsx skill 文件**:侵入性高(影响每个 change),且 SKILL.md 升版时会被覆盖。

自定义 schema 用的是 OpenSpec **原生支持的项目级 schema 机制**:CLI 校验结构、`openspec schemas` 自动列出、每个 change 独立选择 schema(`--schema spec-driven` 或 `--schema superpowers-bridge`)、不修改任何现有 SKILL.md 或 command 文件。
...
| 架构变更 | ✅ 要 | 同上 |
| Bug fix(恢复原本行为,不变更契约) | ❌ 不要 | 直接 PR |
| 测试补写 / 覆盖率 | ❌ 不要 | 直接 PR |
| 构建工具微调(linter 规则、覆盖率阈值等) | ❌ 不要 | 直接 PR |
| 非破坏性依赖升级 | ❌ 不要 | 直接 PR |
| 文档更新 / typo 修正 | ❌ 不要 | 直接 PR |
| Config 值微调(不动结构) | ❌ 不要 | 直接 PR |

> 原则:**流程仪式跟风险成正比**。动到对外契约、跨系统对接、DB schema、合规边界 → 走 change;改 typo、抓 bug、调 timeout 数字 → 直接 PR。模糊地带用下方 5 条 checklist 自我检验。

### 进行中的 verbal brainstorm 何时升级成 change

如果使用者以 narrative(「我们来讨论架构」「头脑风暴一下」)触发了 `superpowers:brainstorming`,brainstorming 的产出**不可以**写到 `docs/superpowers/specs/` —— 那会绕过本 schema 的 output redirection,在 repo 里留下 orphan artifact。
...
> - `retrospective.md` 声明 `requires: verify`,并按 Step 4 在 PR 开启**之前**产出 —— PR diff 才会包含完整 archived cycle(所有 artifact 完成、spec 已 sync、change folder 在 `archive/`)。
...
> **不支持 `executing-plans` fallback**。本 schema 是 opinionated 的:要求 subagent-capable 平台(Claude Code、Codex 等)。替代 executor `superpowers:executing-plans` 并**不会** transitively 触发 TDD 或 code-review(已对 [SKILL.md](https://github.com/obra/superpowers/blob/main/skills/executing-plans/SKILL.md) 做事实查验 —— body 完全没提到 TDD 或 code-review,Integration 段也未列出 `test-driven-development` 与 `requesting-code-review`)。退到 2b 等于静默降级 Superpowers 的核心价值。若你的平台没有 subagent 支持,改用 OpenSpec 内置的 `spec-driven` schema。

### Output redirection(产出重定向)

Superpowers skill 有默认输出路径(例如 brainstorming 写到 `docs/superpowers/specs/`)。本 schema 的 artifact instruction **覆写**这个行为,通过 prompt 上下文注入,把产出重定向到 change 目录:

- brainstorming → `openspec/changes/<name>/brainstorm.md`
- design(必填) → 把 brainstorm 重组为 Context / Goals / Decisions / Risks / Migration
...
skill 缺失 → STOP 并通知使用者,不静默 fallback,本 schema 内也没有 manual mode。建议使用者在那个 change 改用 OpenSpec 内置的 `spec-driven` schema,或安装缺失的 skill 后重来。

> 本 schema 的 v0 版本曾在这里放「自动 commit change artifacts 到当前分支」逻辑,在 [PR #970 review](https://github.com/Fission-AI/OpenSpec/pull/970) 后移除:处理未追踪的 change 目录是 worktree skill 的责任,schema 不应主动改写使用者的 git history。
...
产出 `verify.md`,跑 7 项检查:结构校验(`openspec validate --all --json`)、task 完成度、delta-spec sync 状态、design/specs 一致性(non-blocking warning)、实现信号(commit 状态)、front-door routing leak detector(non-blocking warning)、以及 deferred-dogfood vs automated-test 等价性。最后一项仅在 `plan.md` 有 `[~]` 但等价性章节空白(gap 分析被跳过)时才 block,其他情境属 informational。
...
#### 4. Retrospective —— `retrospective` artifact(建议,按 Entry & exit gates 的 skip 规则 trivial fix 可跳)
...
| 全项目校验 | `openspec validate --all --json` |
...
整合**完全**发生在 `instruction:` 字段(纯 prompt)。Superpowers 升版某个 skill 的行为时,本 schema 不用改。只有 skill 被改名或移除时才要 touch `schema.yaml`。
...
本 schema 要求 subagent-capable 平台(Claude Code、Codex 等)。替代 executor `superpowers:executing-plans` **不会** transitively 触发 TDD 或 code-review(已对其 [SKILL.md](https://github.com/obra/superpowers/blob/main/skills/executing-plans/SKILL.md) 做事实查验 —— body 完全没提及这两者,Integration 段也未列出 `test-driven-development` 与 `requesting-code-review`)。退到 2b 等于静默丢掉 Superpowers 带给整合的核心价值。我们选择在 Step 0 fail loud,并指引使用者改用内置的 `spec-driven` schema。
...
LLM 不必解读 timing 文字 —— 跑命令、看结果即可。这是顾虑 #1 第 2 层,以及顾虑 #2 的缓解。
...
| 标识 | 位置 | 含义 | 示例 |
|---|---|---|---|
| Schema major | `schema.yaml: version: 1` | schema graph 的契约版本(artifacts、`requires:` 边、PRECHECK 形状)。破坏性改动才 bump。 | `1` |
| Bundle release | `VERSION` 文件 + git tag | 此 bundle 的 SemVer 发布版本,从属于某个 schema major。 | `1.0.0`(tag `v1.0.0`) |

bundle release `1.x.y` 是 schema major `v1` 的一个 published cut;未来 schema major `v2` 会把 bundle release 重新从 `2.0.0` 起算。Adopter 钉到 `v1.x.y` 即享有 schema graph 在 v1 major 内的兼容保证。

> 下方兼容矩阵以 `v1`(schema major)为列键,因为与 OpenSpec / Superpowers 的兼容性由 schema 契约决定,不受 bundle 内部 patch 级改动影响。

## 兼容性

本 schema 撰写时所对齐的 upstream 基准版本。这是**历史快照,不是端到端兼容性承诺** —— CI 无法在 headless 环境跑完整的 prompt-layer workflow,所以行为兼容性依赖 drift 触发时的人类检核。
...
Brainstorming 是多轮交互对话,需要使用者参与。把它做为第一个 artifact(而非 schema-level hook)有两个好处:
